...
// Handle POST requests to send a file
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.file) {
      return NextResponse.json(
        { success: false, error: "Missing required parameter: file" },
        { status: 400 }
      )
    }

    // Validar se o file é uma URL válida
    try {
      new URL(body.file);
    } catch {
      return NextResponse.json(
...
//    console.log("Autenticação bem-sucedida. Enviando arquivo...")
    // Call the Evolution API to send the file
    const result = await evolutionApi.sendFile(body.deviceid, body.to, body.message, body.file, apiKeyFromHeader)
